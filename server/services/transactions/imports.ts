...
function importError(message: string, status: number, errors: string[]) {
...
async function importTransactions(payload: ImportTransactionsInput, userId: number): Promise<ImportTransactionsResult> {
  const body = payload || {} as ImportTransactionsInput;
  const errors: string[] = [];
  ...
  if (errors.length > 0) throw importError('Import validation failed', 400, errors);
  ...
  for (const row of billLinkedRows) {
    ...
    if (result.errors?.length) {
      throw importError('Bill payment import failed', 409, [`Row ${row.row_index}: ${result.errors.join(', ')}`]);
    }
