...
function splitTextWithBreaks(text) {
...
function resolveInlineFontFamily(ctx) {
...
function renderInlineText(text, keyPrefix, ctx, extraStyle) {
  const chunks = splitTextWithBreaks(text)
  return chunks.map((chunk, index) => (
    <Text
      key={`${keyPrefix}-${index}`}
      style={[{ fontFamily: resolveInlineFontFamily(ctx) }, extraStyle || null]}
    >
      {chunk}
    </Text>
  ))
...
function renderInlineTokens(tokens, keyPrefix, ctx = { bold: false, italic: false }) {
...
function renderTableCell(cell, key, isHeader) {
  const tokens = Array.isArray(cell?.tokens)
    ? cell.tokens
  ...
  return (
    <Text style={[styles.tableCellText, isHeader ? styles.tableHeaderText : null]}>
...
function renderBlocks(tokens, keyPrefix, inList = false, blockCtx = { textAlign: 'left' }) {
  ...
  return tokens.map((token, index) => {
    ...
    if (token.type === 'table') {
      ...
      const normalizeRow = (row) => Array.from({ length: columnCount }, (_, cellIndex) => row[cellIndex] || { text: '' })
        ...
        <View key={key} style={styles.table}>
          <View style={styles.tableRow}>
            {normalizedHeader.map((cell, cellIndex) => (
              <View
                ...
                style={[styles.tableCell, styles.tableHeaderCell, cellIndex === columnCount - 1 ? styles.tableLastCell : null]}
              >
          ...
          {normalizedRows.map((row, rowIndex) => (
            <View
              ...
              style={[styles.tableRow, rowIndex === normalizedRows.length - 1 ? styles.tableLastRow : null]}
            >
              {row.map((cell, cellIndex) => (
                <View
                  key={`${key}-row-${rowIndex}-cell-${cellIndex}`}
                  style={[styles.tableCell, cellIndex === columnCount - 1 ? styles.tableLastCell : null]}
                >
...
export default function DonationReceiptsPdfDocument({ receipts }) {
