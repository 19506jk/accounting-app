...
export function txFmt(n: number | string | null | undefined) {
...
export default function TransactionTable({
  ...
  columns?: TableColumn<TransactionListItem>[];
  ...
  const baseColumns = useMemo<TableColumn<TransactionListItem>[]>(() => ([
    { key: 'date', label: 'Date',
  ...
  const finalColumns = useMemo(() => {
    ...
    return [
      ...selectedColumns,
      {
        ...
        render: (row: TransactionListItem) => row.is_voided ? null : (
          <Button
            variant="ghost"
            size="sm"
            style={{ color: '#dc2626' }}
